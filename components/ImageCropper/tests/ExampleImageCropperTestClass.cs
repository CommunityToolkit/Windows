...
namespace ImageCropperExperiment.Tests;

[TestClass]
public partial class ExampleImageCropperTestClass : VisualUITestBase
{
    // If you don't need access to UI objects directly or async code, use this pattern.
    [TestMethod]
    public void SimpleSynchronousExampleTest()
    {
        var assembly = typeof(ImageCropper).Assembly;
        var type = assembly.GetType(typeof(ImageCropper).FullName ?? string.Empty);

        Assert.IsNotNull(type, "Could not find ImageCropper type.");
        Assert.AreEqual(typeof(ImageCropper), type, "Type of ImageCropper does not match expected type.");
    }
    ...
    {
        var component = new ImageCropper();
    ...
    [UIThreadTestMethod]
    public void SimpleUIExamplePageTest(ExampleImageCropperTestPage page)
    {
        // You can use the Toolkit Visual Tree helpers here to find the component by type or name:
        var component = page.FindDescendant<ImageCropper>();

        Assert.IsNotNull(component);

        var componentByName = page.FindDescendant("ImageCropperControl");
    ...
    [UIThreadTestMethod]
    public async Task SimpleAsyncUIExamplePageTest(ExampleImageCropperTestPage page)
    {
