...
namespace CameraPreviewExperiment.Tests;

[TestClass]
public partial class ExampleCameraPreviewTestClass : VisualUITestBase
{
    // If you don't need access to UI objects directly or async code, use this pattern.
    [TestMethod]
    public void SimpleSynchronousExampleTest()
    {
        var assembly = typeof(CameraPreview).Assembly;
        var type = assembly.GetType(typeof(CameraPreview).FullName ?? string.Empty);

        Assert.IsNotNull(type, "Could not find CameraPreview type.");
        Assert.AreEqual(typeof(CameraPreview), type, "Type of CameraPreview does not match expected type.");
    }
    ...
    {
        var component = new CameraPreview();
    ...
    [UIThreadTestMethod]
    public void SimpleUIExamplePageTest(ExampleCameraPreviewTestPage page)
    {
        // You can use the Toolkit Visual Tree helpers here to find the component by type or name:
        var component = page.FindDescendant<CameraPreview_ClassicBinding>();

        Assert.IsNotNull(component);

        var componentByName = page.FindDescendant("CameraPreviewControl");
    ...
    [UIThreadTestMethod]
    public async Task SimpleAsyncUIExamplePageTest(ExampleCameraPreviewTestPage page)
    {
    ...
    public async Task ComplexAsyncUIExampleTest()
    {
        await EnqueueAsync(() =>
        {
            var component = new CameraPreview_ClassicBinding();
            Assert.IsNotNull(component);
        });
    }
