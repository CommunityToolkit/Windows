...
[TestClass]
public partial class ExampleSizersTestClass : VisualUITestBase
{
    // If you don't need access to UI objects directly or async code, use this pattern.
    [TestMethod]
    public void SimpleSynchronousExampleTest()
    {
        var assembly = typeof(Sizers).Assembly;
        var type = assembly.GetType(typeof(Sizers).FullName ?? string.Empty);

        Assert.IsNotNull(type, "Could not find Sizers type.");
        Assert.AreEqual(typeof(Sizers), type, "Type of Sizers does not match expected type.");
    }
    ...
    [UIThreadTestMethod]
    public void SimpleUIExamplePageTest(ExampleSizersTestPage page)
    {
        // You can use the Toolkit Visual Tree helpers here to find the component by type or name:
        var component = page.FindDescendant<Sizers_ClassicBinding>();

        Assert.IsNotNull(component);

        var componentByName = page.FindDescendant("SizersControl");
    ...
    [UIThreadTestMethod]
    public async Task SimpleAsyncUIExamplePageTest(ExampleSizersTestPage page)
    {
    ...
    public async Task ComplexAsyncUIExampleTest()
    {
        await EnqueueAsync(() =>
        {
            var component = new Sizers_ClassicBinding();
            Assert.IsNotNull(component);
        });
    }
