...
using CommunityToolkit.WinUI.Media.Geometry.Core;
using CommunityToolkit.WinUI.Media.Geometry.Parsers;
using Windows.UI;

namespace CommunityToolkit.WinUI.Media.Geometry.Elements.Brush;
    ...
    protected override void GetAttributes(Match match)
    {
        ...
        if (group.Success)
        {
            ...
            foreach (Capture capture in group.Captures)
            {
                ...
                if (main.Success)
                {
                    var regex = RegexFactory.GetAttributesRegex(GradientStopAttributeType.Main);
                    default(ArgumentNullException).ThrowIfNull(regex);

                    var mainMatch = regex.Match(main.Value);
                ...
                {
                    var regex = RegexFactory.GetAttributesRegex(GradientStopAttributeType.Additional);
                    default(ArgumentNullException).ThrowIfNull(regex);

                    var addMatch = regex.Match(addCapture.Value);
