...
namespace IncrementalLoadingCollectionExperiment.Tests;

[TestClass]
public partial class ExampleIncrementalLoadingCollectionTestClass : VisualUITestBase
{
    // If you don't need access to UI objects directly or async code, use this pattern.
    [TestMethod]
    public void SimpleSynchronousExampleTest()
    {
        var assembly = typeof(IncrementalLoadingCollection).Assembly;
        var type = assembly.GetType(typeof(IncrementalLoadingCollection).FullName ?? string.Empty);

        Assert.IsNotNull(type, "Could not find IncrementalLoadingCollection type.");
        Assert.AreEqual(typeof(IncrementalLoadingCollection), type, "Type of IncrementalLoadingCollection does not match expected type.");
    }
    ...
    {
        var component = new IncrementalLoadingCollection();
    ...
    [UIThreadTestMethod]
    public void SimpleUIExamplePageTest(ExampleIncrementalLoadingCollectionTestPage page)
    {
        // You can use the Toolkit Visual Tree helpers here to find the component by type or name:
        var component = page.FindDescendant<IncrementalLoadingCollection_ClassicBinding>();

        Assert.IsNotNull(component);

        var componentByName = page.FindDescendant("IncrementalLoadingCollectionControl");
    ...
    [UIThreadTestMethod]
    public async Task SimpleAsyncUIExamplePageTest(ExampleIncrementalLoadingCollectionTestPage page)
    {
    ...
    public async Task ComplexAsyncUIExampleTest()
    {
        await EnqueueAsync(() =>
        {
            var component = new IncrementalLoadingCollection_ClassicBinding();
            Assert.IsNotNull(component);
        });
    }
