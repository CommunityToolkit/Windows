...
[TestClass]
public partial class ExampleRadialGaugeTestClass : VisualUITestBase
{
    // If you don't need access to UI objects directly or async code, use this pattern.
    [TestMethod]
    public void SimpleSynchronousExampleTest()
    {
        var assembly = typeof(RadialGauge).Assembly;
        var type = assembly.GetType(typeof(RadialGauge).FullName ?? string.Empty);

        Assert.IsNotNull(type, "Could not find RadialGauge type.");
        Assert.AreEqual(typeof(RadialGauge), type, "Type of RadialGauge does not match expected type.");
    }
    ...
    [UIThreadTestMethod]
    public async Task SimpleAsyncUIExamplePageTest(ExampleRadialGaugeTestPage page)
    {
